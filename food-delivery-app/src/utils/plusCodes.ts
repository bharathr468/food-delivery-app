/*
 * Taken from https://github.com/tspoke/typescript-open-location-code/blob/master/src/open-location-code.ts
 */
...
export default class OpenLocationCode {
	...
	private static readonly PAIR_RESOLUTIONS_ = [20.0, 1.0, 0.05, 0.0025, 0.000125];

	// First place value of the pairs (if the last pair value is 1).
	private static readonly PAIR_FIRST_PLACE_VALUE_ = Math.pow(
		OpenLocationCode.ENCODING_BASE_,
		OpenLocationCode.PAIR_CODE_LENGTH_ / 2 - 1
	);
	...
	private static readonly GRID_LAT_FIRST_PLACE_VALUE_ = Math.pow(
		OpenLocationCode.GRID_ROWS_,
		OpenLocationCode.GRID_CODE_LENGTH_ - 1
	);

	// First place value of the longitude grid (if the last place is 1).
	private static readonly GRID_LNG_FIRST_PLACE_VALUE_ = Math.pow(
		OpenLocationCode.GRID_COLUMNS_,
		OpenLocationCode.GRID_CODE_LENGTH_ - 1
	);
	...
	private static readonly FINAL_LAT_PRECISION_ =
		OpenLocationCode.PAIR_PRECISION_ *
	...
	public static isValid(code: string): boolean {
		...
		if (
			code.indexOf(OpenLocationCode.SEPARATOR_) > OpenLocationCode.SEPARATOR_POSITION_ ||
		...
		const strippedCode = code
			.replace(new RegExp('\\' + OpenLocationCode.SEPARATOR_ + '+'), '')
	...
	public static isShort(code: string): boolean {
		...
		return (
			code.indexOf(OpenLocationCode.SEPARATOR_) >= 0 &&
			code.indexOf(OpenLocationCode.SEPARATOR_) < OpenLocationCode.SEPARATOR_POSITION_
		);
	}
	...
	public static isFull(code: string): boolean {
		...
		const firstLatValue =
			OpenLocationCode.CODE_ALPHABET_.indexOf(code.charAt(0).toUpperCase()) * OpenLocationCode.ENCODING_BASE_;
		...
		if (code.length > 1) {
			...
			const firstLngValue =
				OpenLocationCode.CODE_ALPHABET_.indexOf(code.charAt(1).toUpperCase()) * OpenLocationCode.ENCODING_BASE_;
	...
	public static encode(
		latitude: number,
		longitude: number,
		codeLength: number = OpenLocationCode.CODE_PRECISION_NORMAL
	): string {
		...
		let latVal = Math.floor(
			Math.round((editedLatitude + OpenLocationCode.LATITUDE_MAX_) * OpenLocationCode.FINAL_LAT_PRECISION_ * 1e6) / 1e6
		);
		let lngVal = Math.floor(
			Math.round((editedLongitude + OpenLocationCode.LONGITUDE_MAX_) * OpenLocationCode.FINAL_LNG_PRECISION_ * 1e6) /
				1e6
		);
		...
		code =
			code.substring(0, OpenLocationCode.SEPARATOR_POSITION_) +
		...
		return (
			code.substring(0, editedCodeLength) +
			Array(OpenLocationCode.SEPARATOR_POSITION_ - editedCodeLength + 1).join(OpenLocationCode.PADDING_CHARACTER_) +
			OpenLocationCode.SEPARATOR_
		);
	}
	...
	public static decode(code: string): CodeArea {
		...
		const editedCode = code
			.replace(OpenLocationCode.SEPARATOR_, '')
	...
	public static recoverNearest(shortCode: string, latitude: number, longitude: number): string {
		...
		const codeArea = OpenLocationCode.decode(
			OpenLocationCode.encode(referenceLatitude, referenceLongitude).substr(0, paddingLength) + shortCodeUpper
		);
	...
	public static shorten(code: string, latitude: number, longitude: number): string {
		...
		const range = Math.max(
			Math.abs(codeArea.latitudeCenter - latitudeClipped),
			Math.abs(codeArea.longitudeCenter - longitudeClipped)
		);
